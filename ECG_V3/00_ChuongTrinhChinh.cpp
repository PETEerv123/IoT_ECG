...
void ChayChuongTrinhChinh() {
  if (_WiFi.DaBatAP != 1) {
    if (millis() - Esp32delaytime >= delaytime)  //test whether the period has elapsed
    {
      ...
      float voltage = adcValue * (3 / 4095.0); // 3v3 đầu vào mà sửa lại 3V cho chính xác thực tế :v

      // Tính VBAT dựa trên mạch chia áp
      float vbat = voltage * ((R1 + R2) / R2);
      int8_t percBat = int8_t((vbat - 3.2) * 100.0 / (4.2 - 3.2));
      percBat = constrain(percBat, 0, 100);  // Giới hạn từ 0 đến 100%
