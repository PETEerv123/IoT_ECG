...
MAX30003::MAX30003(int cs_pin){ 
    // : notchFilter(7, 512.0, 30) { // f0=5Hz, fs=512Hz, Q=20
    _cs_pin = cs_pin;
    pinMode(_cs_pin, OUTPUT);
    digitalWrite(_cs_pin, HIGH);
}
...
void MAX30003::max30003Begin() {
  ...
  max30003RegWrite(CNFG_GEN, 0x81D5B);  // Dòng điện DC Lead-Off: 100nA, ngưỡng phát hiện: VMID ± 400mV Bia 200M
  delay(100);

  // Cấu hình hiệu chuẩn (Calibration) CNFG_CAL
  // Cài đặt tín hiệu chuẩn để kiểm tra đường tín hiệu
  max30003RegWrite(CNFG_CAL, 0x700800);        //   Calibration Source Magnitude Selection (VMAG)  = 0.50mV  || Calibration Source Frequency 256Hz
  // Cấu hình MUX đầu vào CNFG_EMUX 0x304800 0x700800
  ...
  // Cấu hình CNFG_RTOR1
  max30003RegWrite(CNFG_RTOR1, 0x3FA600);  //0x5FC600 0x3fc600 0x3FA300
  delay(100);
  // Đồng bộ hóa cài đặt
  max30003Synch();
  delay(100);
...
void MAX30003::getEcgSamples(void) {
  ...
  // signed long rawValue = (signed long)data;

    // Áp dụng bộ lọc Notch
  // double filteredValue = notchFilter.process((double)rawValue);

    // Gán giá trị đã lọc vào biến ecgdata
  // ecgdata = (signed long)filteredValue;
// #ifdef debug
